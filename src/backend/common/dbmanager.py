...
class DBManager(object):
    ...
    def get_paper_ids_flagged_positive_svm(self, svm_filters):
        self.cur.execute("SELECT cur_paper FROM cur_blackbox WHERE cur_datatype IN %s AND UPPER(cur_blackbox) IN "
                         "('HIGH', 'MEDIUM')", (svm_filters,))
    ...
    def query_papers_list(self, query, svm_filters=None, manual_filters=None, curation_filters=None,
        ...
        if manual_filters and manual_filters[0] != '':
            additional_where_clause += " AND (" + (combine_filters + " ").join(
        ...
        query = query + additional_where_clause
